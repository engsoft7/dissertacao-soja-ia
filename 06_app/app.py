...
with dir_:
    ...
    # Base do Gráfico
    base = alt.Chart(serie_plot).encode(x=alt.X("ano:O", title="Ano-safra"))
    ...
    # 2. NOVA FEATURE: Linha de Tendência (Regressão Linear)
    ...
    # 4. NOVA FEATURE: Faixas de El Niño / La Niña
    ...
    b1, b2 = st.columns(2)
    b1.download_button("Baixar série do município (CSV)", serie.to_csv(index=False).encode("utf-8"), file_name=f"soja_{municipio.lower().replace(' ', '_')}.csv", mime="text/csv", width='stretch')
    b2.download_button("Baixar base completa (CSV)", DADOS.read_bytes(), file_name=DADOS.name, mime="text/csv", width='stretch')
